...
DROP DATABASE IF EXISTS wtrlite##

CREATE DATABASE IF NOT EXISTS wtrlite_test##
CREATE DATABASE IF NOT EXISTS wtrlite##
...
CREATE TABLE `Report` (
    ...
    `status` varchar(999)  NOT NULL DEFAULT 'confirmed' ,
    PRIMARY KEY (
        `reportId`
    )
) engine=InnoDB##
...
DROP PROCEDURE IF EXISTS `get_list_of_factor`##



DROP PROCEDURE IF EXISTS `get_list_of_user`##
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_list_of_factor` (IN inId INT)
BEGIN
   SELECT * FROM factor WHERE factorId like inId;
END
##



CREATE DEFINER=`root`@`localhost` PROCEDURE `get_list_of_user` (IN inName VARCHAR(999))
BEGIN
   SELECT * FROM user WHERE userName like inName;
END
##
